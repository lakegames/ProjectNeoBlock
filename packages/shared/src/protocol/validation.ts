...
export function validateCommand(input: unknown): ValidationResult<Command> {
  ...
  const requireString = (key: string) => {
    const v = input[key];
    if (!isString(v) || v.length === 0) issues.push(issue(key, `${key} 必须为非空字符串`));
    return v;
  };
  ...
  switch (type) {
    case 'room/join': {
      ...
      if (!(resume === undefined || (isNumber(resume) && resume >= 0)))
        issues.push(issue('resumeFromSeqExclusive', 'resumeFromSeqExclusive 必须为非负数'));
      break;
    }
    case 'room/leave': {
...
export function validateEvent(input: unknown): ValidationResult<Event> {
  ...
  if (!isNumber(input.seq) || input.seq < 0) issues.push(issue('seq', 'seq 必须为非负数'));
...
export function validateMatchSnapshot(input: unknown): ValidationResult<MatchSnapshot> {
  const issues: ValidationIssue[] = [];
  ...
  if (
    ...
    issues.push(issue('cursor.lastEventSeq', 'cursor.lastEventSeq 必须为非负数'));
  }
  if (issues.length > 0) return fail(issues);
...
export function validateServerMessage(input: unknown): ValidationResult<ServerMessage> {
  if (!isRecord(input)) return fail([issue('', 'message 必须为对象')]);
  ...
  if (input.kind === 'events') {
    ...
    if (!isArray(input.events)) issues.push(issue('events', 'events 必须为数组'));
    if (issues.length > 0) return fail(issues);
  ...
  if (input.kind === 'error') {
    const issues: ValidationIssue[] = [];
    if (!isRecord(input.error)) return fail([issue('error', 'error 必须为对象')]);
    if (!isString(input.error.code)) issues.push(issue('error.code', 'error.code 必须为字符串'));
