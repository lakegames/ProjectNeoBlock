...
function findSeed(
  predicate: (dice1: [number, number], dice2: [number, number]) => boolean,
) {
...
function findSeed4(
  predicate: (d1: number, d2: number, d3: number, d4: number) => boolean,
) {
...
test("事件驱动：买地、成套双倍租金 + 回放一致性", () => {
  ...
  const seed = findSeed4(
    (d1, d2, d3, d4) =>
      d1 % 3 === 1 && d2 % 3 === 0 && d3 % 3 === 1 && d4 % 3 === 2,
  );
...
test("抵押：抵押地产后免收租金", () => {
  ...
  const seed = findSeed(
    (d1, d2) => (d1[0] + d1[1]) % 2 === 1 && (d2[0] + d2[1]) % 2 === 1,
  );
...
test("建房：必须均衡建造，且支持升级旅馆", () => {
  ...
  const seed = findSeed3(
    (d1, d2, d3) => d1 % 3 === 1 && d2 % 3 === 0 && d3 % 3 === 1,
  );
...
test("拍卖：放弃购买会进入拍卖，最高价赢得资产", () => {
  ...
  state = handleCommand(
    state,
    {
      ...
      roomId: "room-4",
      gameId: "game-4",
      playerId: p2Prompt.playerId,
      promptId: p2Prompt.promptId,
      choice: { pass: true },
    },
    5,
  ).state;
