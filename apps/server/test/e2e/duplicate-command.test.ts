...
describe('e2e: duplicate command idempotency', () => {
  ...
  before(async () => {
    server = await startServer({ port: 0, boardPreset: 'e2e_fast', fixedGameSeed: 'e2e-dup-seed', initialCash: 200 });
  });
  ...
  it('sending same commandId twice returns DUPLICATE_COMMAND', async () => {
    ...
    try {
...

      {
        ...
        p1.sendCommand({ type: 'room/setReady', roomId, playerId: 'u1', ready: true, commandId });
        await p1.waitForSnapshot({ predicate: (s) => s.cursor.lastEventSeq > prev });
      }

      p1.sendCommand({ type: 'room/setReady', roomId, playerId: 'u1', ready: true, commandId, clientSeq: 0 });
      const err = await p1.waitForError({ predicate: (e) => e.commandId === commandId });
