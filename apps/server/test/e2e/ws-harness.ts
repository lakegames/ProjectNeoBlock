...
export class WsHarnessClient {
  ...
  get playerId() {
    const id = this.lastSnapshot?.room.members.find(
      (m) => m.userId === this.userId || m.playerId === this.userId,
    )?.playerId;
  ...
  async connect(input?: { resumeFromSeqExclusive?: number }) {
    ...
    ws.on("message", (data) => {
      ...
      if (!validated.ok)
        throw new Error(
          `INVALID_SERVER_MESSAGE:${validated.issues.map((i) => `${i.path}:${i.message}`).join(",")}`,
        );
    ...
    const joinCommand: Extract<Command, { type: "room/join" }> = {
      ...
      mode: this.mode,
      ...(input?.resumeFromSeqExclusive !== undefined
        ? { resumeFromSeqExclusive: input.resumeFromSeqExclusive as any }
        : {}),
    };
  ...
  async waitForSnapshot(input?: {
    predicate?: (s: MatchSnapshot) => boolean;
    timeoutMs?: number;
  }) {
    ...
    const msg = await this.waitForMessage(
      (m) => m.kind === "snapshot" && predicate(m.snapshot),
      timeoutMs,
    );
    return (msg as Extract<ServerMessage, { kind: "snapshot" }>).snapshot;
  }

  async waitForError(input?: {
    predicate?: (e: ProtocolError) => boolean;
    timeoutMs?: number;
  }) {
    const timeoutMs = input?.timeoutMs ?? 5_000;
    const predicate = input?.predicate ?? (() => true);
    ...
    const msg = await this.waitForMessage(
      (m) => m.kind === "error" && predicate(m.error),
      timeoutMs,
    );
    return (msg as Extract<ServerMessage, { kind: "error" }>).error;
  }

  sendCommand<T extends Command>(
    command: Omit<T, "commandId" | "clientSeq"> &
      Partial<Pick<T, "commandId" | "clientSeq">>,
  ) {
