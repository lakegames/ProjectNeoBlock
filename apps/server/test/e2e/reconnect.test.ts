...
describe('e2e: reconnect resume', () => {
  ...
  it('disconnect then reconnect with resumeFromSeqExclusive receives missed events', async () => {
    ...
    try {
      ...
      await p1.waitForSnapshot({ predicate: (s) => s.cursor.lastEventSeq > seqBefore });
      ...
      const eventsMsg = p2r.waitForMessage((m) => m.kind === 'events' && m.fromSeqExclusive >= seqBefore, 5_000);
