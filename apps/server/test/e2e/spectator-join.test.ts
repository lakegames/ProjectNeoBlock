...
describe('e2e: spectator join mid-game', () => {
  ...
  before(async () => {
    server = await startServer({ port: 0, boardPreset: 'e2e_fast', fixedGameSeed: 'e2e-spectator-seed', initialCash: 200 });
  });
  ...
  it('spectator can join and receives snapshot; cannot issue game commands', async () => {
    ...
    const spectator = new WsHarnessClient({ url, roomCode, userId: 's1', displayName: 'S1', mode: 'spectator' });
    ...
    try {
      ...
      const roomId = p1.lastSnapshot!.room.roomId;
      {
        ...
        p1.sendCommand({ type: 'room/setReady', roomId, playerId: 'u1', ready: true });
        await p1.waitForSnapshot({ predicate: (s) => s.cursor.lastEventSeq > prev });
      }
      {
        const prev = p2.lastEventSeq;
        p2.sendCommand({ type: 'room/setReady', roomId, playerId: 'u2', ready: true });
        await p2.waitForSnapshot({ predicate: (s) => s.cursor.lastEventSeq > prev });
      }
      {
        p1.sendCommand({ type: 'room/startGame', roomId, playerId: 'u1' });
        await p1.waitForSnapshot({ predicate: (s) => s.room.status === 'playing' });
      }
