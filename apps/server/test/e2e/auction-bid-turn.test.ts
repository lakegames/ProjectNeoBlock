...
async function waitApplied(input: { client: WsHarnessClient; command: Parameters<WsHarnessClient['sendCommand']>[0]; observer: WsHarnessClient }) {
  const prevSeq = input.observer.lastEventSeq;
  const sent = input.client.sendCommand(input.command as any);
  await Promise.race([
    input.observer.waitForSnapshot({ predicate: (s) => s.cursor.lastEventSeq > prevSeq }),
    input.client.waitForError({ predicate: (e) => e.commandId === sent.commandId }).then((e) => Promise.reject(new Error(`SERVER_ERROR:${e.code}:${e.message}`))),
  ]);
}
...
describe('e2e: auction bid rotation', () => {
  ...
  it('second bidder can bid and prompt rotates correctly', async () => {
    ...
    try {
      ...
      await waitApplied({ client: p1, observer: p1, command: { type: 'room/setReady', roomId, playerId: 'u1', ready: true } });
      await waitApplied({ client: p2, observer: p1, command: { type: 'room/setReady', roomId, playerId: 'u2', ready: true } });
      await waitApplied({ client: p1, observer: p1, command: { type: 'room/startGame', roomId, playerId: 'u1' } });

      let snapshot = p1.lastSnapshot!;
      let auctionStarter: string | null = null;
      for (let i = 0; i < 20; i++) {
        const game = snapshotGame(snapshot);
        if (game.phase === 'await_prompt' && game.engineState.pendingPrompt?.kind === 'buyOrAuction') break;
        if (game.phase === 'await_roll') {
          const current = game.currentPlayerId;
          const client = current === 'u1' ? p1 : p2;
          await waitApplied({ client, observer: p1, command: { type: 'game/rollDice', roomId, gameId: game.gameId, playerId: current } });
        } else if (game.phase === 'await_end_turn') {
          const current = game.currentPlayerId;
          const client = current === 'u1' ? p1 : p2;
          await waitApplied({ client, observer: p1, command: { type: 'game/endTurn', roomId, gameId: game.gameId, playerId: current } });
        } else {
