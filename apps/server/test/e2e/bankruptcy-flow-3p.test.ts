...
async function stepOnce(input: {
  clientByPlayerId: Map<string, WsHarnessClient>;
...
describe("e2e: bankruptcy settlement flow (3 players)", () => {
  let server: RunningServer;
  const roomCode = "E2E9E";

  before(async () => {
    server = await startServer({
      port: 0,
      boardPreset: "e2e_fast",
      fixedGameSeed: "e2e-fast-3p-seed",
  ...
  it("3 players play until game ended (at least 1 eliminated)", async () => {
    ...
    try {
...

      {
        ...
        await Promise.race([
          p1.waitForSnapshot({
            predicate: (s) => s.room.status === "playing" && s.game !== null,
          }),
          p1
            .waitForError({ predicate: (e) => e.commandId === c.commandId })
            .then((e) => Promise.reject(new Error(e.code))),
        ]);
      }
