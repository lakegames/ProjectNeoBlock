...
export function ensureSeedConfigs(data: AppData, nowMs: number) {
  const hasPublished = (kind: ConfigKind) => Object.values(data.configDocs).some((d) => d.kind === kind && d.publishedVersionId);
...
export function createAppDataStore(): AppDataStore {
    ...
    loadPromise = (async () => {
      ...
      try {
        ...
        cache = {
          ...
          rooms: parsed.rooms ?? {},
          configDocs:
            typeof (parsed as { configDocs?: unknown }).configDocs === 'object' && (parsed as { configDocs?: unknown }).configDocs !== null
              ? ((parsed as { configDocs: Record<string, ConfigDoc> }).configDocs as Record<string, ConfigDoc>)
              : {},
  ...
  const flush = async () => {
    await ensureDataDir();
    await writeFile(dataFilePath(), JSON.stringify(cache ?? defaultData, null, 2), 'utf8');
  };
