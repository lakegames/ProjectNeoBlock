...
function shouldCloseRoomByEmpty(
  room: { members: unknown[]; emptySinceMs?: number; closedAtMs?: number },
  nowMs: number,
) {
...
function proxyAuthOk(req: IncomingMessage) {
  ...
  if (!expected)
    return {
      ok: false as const,
      status: 500,
      body: { error: "PROXY_KEY_NOT_CONFIGURED" },
    };
...
function ensureUser(req: IncomingMessage) {
  ...
  const displayName =
    normalizeDisplayName(
      getHeader(req, "x-neoblock-actor-display-name") || uid,
    ) || uid;
  const githubAvatarUrl = validateGithubAvatarUrl(
    getHeader(req, "x-neoblock-actor-github-avatar-url"),
  );
...
function ensureGuest(
  ...
  const nickname = normalizeDisplayName(
    inputNickname || headerNickname || "游客",
  );
...
export function createAppDataHttpHandler(store: AppDataStore) {
  return async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (pathname === "/api/config/published" && req.method === "GET") {
      ...
      const hasDefaultTpl = !!(
        sysTpl?.publishedVersionId || legacyTpl?.publishedVersionId
      );
      if (
        !Object.keys(data.configDocs).length ||
        !full ||
        !full.publishedVersionId ||
        !hasDefaultTpl
      ) {
      ...
      const defaultTemplateVersionId =
        sysTpl2?.publishedVersionId || legacyTpl2?.publishedVersionId || null;

      const hiddenTemplateDocIds = new Set([
        "system:template-standard",
        "builtin:template-standard",
      ]);
    ...
    if (pathname === "/api/room/create" && req.method === "POST") {
      ...
      const result = await store.update((data) => {
        ...
        if (!guest)
          return { ok: false as const, error: "INVALID_NICKNAME" as const };
    ...
    if (pathname === "/api/room/join" && req.method === "POST") {
      ...
      const result = await store.update((data) => {
        ...
        if (mode === "account") {
          ...
          const isExisting = room.members.some(
            (m) => m.playerId === playerId && !m.isSpectator,
          );
    ...
    if (pathname === "/api/room/state" && req.method === "GET") {
      const roomCode =
        url.searchParams.get("roomCode")?.trim().toUpperCase() ?? "";
      if (!roomCode) {
        sendJson(res, 400, { error: "INVALID_ROOM_CODE" });
        return;
      }
      ...
      const selfMember = actor
        ? (result.room.members.find((m) => m.playerId === actor.playerId) ??
          null)
        : null;
      sendJson(res, 200, {
        roomCode,
    ...
    if (pathname === "/api/profile" && req.method === "GET") {
      ...
      const result = await store.update((data) => {
        ...
        if (
          ...
          actor.githubAvatarUrl !== data.profiles[actor.uid]!.githubAvatarUrl
        ) {
    ...
    if (pathname === "/api/profile/public" && req.method === "GET") {
      ...
      const profiles = ids.map((id) => {
        ...
        const avatar = p
          ? avatarInfoFor(p)
          : { avatarKind: "none" as const, avatarUrl: null };
        return {
          id,
