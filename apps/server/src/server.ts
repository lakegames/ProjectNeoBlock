...
export async function startServer(
  ...
  const reconnectWindowMs =
    options.reconnectWindowMs ??
    Number(process.env.RECONNECT_WINDOW_MS ?? 30_000);
  const maxEvents =
    options.maxEvents ?? Number(process.env.MAX_EVENTS ?? 2_000);
  const boardPreset =
    options.boardPreset ??
    (process.env.BOARD_PRESET as BoardPreset | undefined) ??
    "default";
  const fixedGameSeed = options.fixedGameSeed ?? process.env.FIXED_GAME_SEED;
  const initialCash =
    options.initialCash ??
    (process.env.INITIAL_CASH ? Number(process.env.INITIAL_CASH) : undefined);
  ...
  function allocEventBase(
    room: RoomState,
    input: { causedBy?: { commandId?: CommandId; playerId?: PlayerId } },
  ) {
  ...
  function closeRoom(room: RoomState) {
    ...
    for (const timer of room.pendingDisconnectTimers.values())
      clearTimeout(timer);
  ...
  function onSocketClosed(ws: WebSocket) {
    ...
    if (room.playerToSocket.get(playerId) === ws)
    ...
    const timer = setTimeout(() => {
      ...
      if (r.hostPlayerId === playerId && r.members.size > 0) {
        const ordered = [...r.members.values()].sort(
          (a, b) => a.joinedAtMs - b.joinedAtMs,
        );
        const nextHost =
          ordered.find((x) => !x.isSpectator)?.playerId ?? ordered[0]!.playerId;
  ...
  async function handleRoomCommand(
    ...
    if (room.processedCommandIds.has(commandId)) {
      sendError(
        ws,
        createProtocolError({
          code: ErrorCode.DUPLICATE_COMMAND,
          message: "重复指令",
          commandId,
        }),
      );
    ...
    let pendingClientSeq: { playerId: PlayerId; clientSeq: number } | null =
      null;
    ...
    const requireClientSeq = () => {
      ...
      if (command.clientSeq !== expected)
        throw createProtocolError({
          ...
          commandId,
          details: {
            expectedClientSeq: expected,
            gotClientSeq: command.clientSeq,
            lastClientSeq: last,
          },
        });
      pendingClientSeq = {
    ...
    const commitClientSeq = () => {
      ...
      room.lastClientSeqByPlayer.set(
        pendingClientSeq.playerId,
    ...
    if (command.type === "room/leave") {
      ...
      if (existed) {
        const base = allocEventBase(room, {
          causedBy: { commandId, playerId: conn.playerId },
        });
        const left: Event = {
          ...base,
    ...
    if (command.type === "room/setReady") {
      ...
      const base = allocEventBase(room, {
        causedBy: { commandId, playerId: command.playerId },
      });
      const e: Event = {
        ...base,
    ...
    if (command.type === "room/setConfig") {
      ...
      if (
        typeof patch.maxPlayers === "number" &&
        Number.isFinite(patch.maxPlayers)
      ) {
        const next = Math.trunc(patch.maxPlayers);
        if (next < 2 || next > 16)
          throw createProtocolError({
            code: ErrorCode.VALIDATION_FAILED,
            message: "maxPlayers 范围为 2-16",
            commandId,
          });
        room.config.maxPlayers = next;
      }
      if (
        patch.boardPreset === "default" ||
        patch.boardPreset === "full" ||
        patch.boardPreset === "e2e_fast"
      ) {
    ...
    if (command.type.startsWith("game/")) {
      ...
      if (!room.game)
        throw createProtocolError({
          code: ErrorCode.GAME_NOT_STARTED,
          message: "对局未开始",
          commandId,
        });
      ...
      if ("playerId" in cAny)
        requireSelfPlayer((cAny as { playerId: PlayerId }).playerId);
  ...
  async function handleJoinCommand(
    command: Extract<Command, { type: "room/join" }>,
    conn: ConnectionState,
    ws: WebSocket,
  ) {
    ...
    const playerId: PlayerId = (
      userId && userId.length > 0 ? userId : `guest:${crypto.randomUUID()}`
    ) as PlayerId;
    ...
    if (prevSocket && prevSocket !== ws)
      closeSocket(prevSocket, 4000, "replaced");
    ...
    if (member) {
      ...
      if (!member.connected) {
        ...
        const base = allocEventBase(room, {
          causedBy: { commandId: command.commandId, playerId },
        });
        const e: Event = {
          ...base,
          type: "room/playerConnectionChanged",
      ...
      const playerCount = [...room.members.values()].filter(
        (m) => !m.isSpectator,
      ).length;
    ...
    if (resume !== null) {
      ...
      const fromSeqExclusive = Math.max(
        resume,
        room.minEventSeq - 1,
      ) as EventSeq;
