...
async function apiJson<T>(url: string) {
  ...
  if (!r.ok)
    throw new Error(
      String((json as { error?: unknown } | null)?.error || r.status),
    );
  return json as T;
}
...
export default function AdminPage() {
  ...
  const selectedRoom = useMemo(
    () => rooms.find((r) => r.roomCode === selectedRoomCode) ?? null,
  ...
  async function refreshRooms() {
    const res = await apiJson<{ rooms: AdminRoom[] }>(
      "/api/admin/room/list?includeClosed=1",
    );
    setRooms(res.rooms);
    if (
      selectedRoomCode &&
      !res.rooms.some((r) => r.roomCode === selectedRoomCode)
    )
      setSelectedRoomCode("");
  }
  ...
  async function refreshAudit() {
    const res = await apiJson<{ audit: AuditEntry[] }>(
      "/api/admin/audit/list?limit=200",
    );
    setAudit(res.audit);
  }
  ...
  useEffect(() => {
    ...
    setError(null);
    if (tab === "rooms")
      void refreshRooms().catch((e) =>
        setError(String((e as Error).message || e)),
      );
    if (tab === "users")
      void refreshUsers().catch((e) =>
        setError(String((e as Error).message || e)),
      );
    if (tab === "audit")
      void refreshAudit().catch((e) =>
        setError(String((e as Error).message || e)),
      );
  }, [adminOk, tab]);
  ...
  return (
    <main style={{ padding: 24, maxWidth: 1100 }}>
      ...
      {!uid ? (
        ...
        <div style={{ marginTop: 20, color: "rgba(0,0,0,0.7)" }}>
          检查权限中…
        </div>
      ) : !adminOk ? (
          ...
          {tab === "rooms" ? (
            ...
            >
                    ...
                    <div style={{ display: "grid", gap: 8 }}>
                      {rooms.map((r) => (
                        ...
                        >
                          ...
                          >
                            host: {r.hostDisplayName} · 玩家 {r.playerCount} ·
                            观战 {r.spectatorCount}
                          </div>
                          <div
                            style={{
                          ...
                          >
                            created: {fmtMs(r.createdAtMs)} · closed:{" "}
                            {fmtMs(r.closedAtMs)}
                          </div>
                        </button>
                      ))}
              ...
              >
                ...
                {!selectedRoom ? (
                  <div style={{ marginTop: 10, color: "rgba(0,0,0,0.6)" }}>
                    选择左侧房间查看
                  </div>
                ) : (
                  <div style={{ marginTop: 10 }}>
                    <div style={{ color: "rgba(0,0,0,0.65)", fontSize: 13 }}>
                      status: {selectedRoom.status} · roomId:{" "}
                      {selectedRoom.roomId}
                    </div>
                    <div
                      style={{
                    ...
                    >
                      started: {fmtMs(selectedRoom.startedAtMs)} · ended:{" "}
                      {fmtMs(selectedRoom.endedAtMs)} · closed:{" "}
                      {fmtMs(selectedRoom.closedAtMs)}
                    </div>
                    ...
                    >
                      ...
                      <Button
                        onClick={() =>
                          act(() =>
                            apiPost("/api/admin/room/archive", {
                              roomCode: selectedRoom.roomCode,
                            }).then(() => void 0),
                          )
                        }
                      ...
                      <Button
                        onClick={() =>
                          act(() =>
                            apiPost("/api/admin/room/restore", {
                              roomCode: selectedRoom.roomCode,
                            }).then(() => void 0),
                          )
                        }
                      ...
                      <Button
                        onClick={() =>
                          act(() =>
                            apiPost("/api/admin/room/force-end", {
                              roomCode: selectedRoom.roomCode,
                              reason: "admin",
                            }).then(() => void 0),
                          )
                      ...
                      <div style={{ marginTop: 8, display: "grid", gap: 8 }}>
                        {selectedRoom.members.map((m) => (
                          ...
                          >
                            ...
                            >
                              {m.isSpectator ? "spectator" : "player"} · ready:{" "}
                              {String(m.ready)}
                            </div>
                            <div
                              style={{
                            ...
                            >
                              <Button
                                onClick={() =>
                                  act(() =>
                                    apiPost("/api/admin/room/set-host", {
                                      roomCode: selectedRoom.roomCode,
                                ...
                                disabled={
                                  busy ||
                                  selectedRoom.hostPlayerId === m.playerId
                                }
                              >
                                设为房主
                              </Button>
              ...
              <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
                {audit.map((a) => (
                  ...
                  >
                    ...
                    >
                      actor: {a.actorUid} · target: {a.targetType ?? "-"}{" "}
                      {a.targetId ?? ""}
                    </div>
                    <pre
                      style={{
