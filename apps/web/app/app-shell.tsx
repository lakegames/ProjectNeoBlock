...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  const displayName =
    profile?.displayName ?? data?.user?.name ?? data?.user?.email ?? "";
  ...
  async function saveDisplayName() {
    ...
    try {
      ...
      setDisplayNameDraft(
        String(json?.profile?.displayName ?? displayNameDraft),
      );
    } catch (e) {
  ...
  return (
    <div className="nb-app-shell" data-nb-app-shell>
          ...
          <div className="nb-app-shell__status" data-nb-status>
            {pathname === "/" && activeGameRoomCode ? (
              <Button
                ...
                onClick={() =>
                  router.push(`/room/${encodeURIComponent(activeGameRoomCode)}`)
                }
              >
                你还有游戏进行中
              </Button>
      ...
      <div className="nb-app-shell__content" data-nb-content>
        ...
        {profileOpen ? (
          <aside
            className="nb-app-shell__profile-sidebar"
            data-nb-profile-sidebar
          >
            <div
              className="nb-app-shell__profile-sidebar-header"
              data-nb-profile-sidebar-header
            >
              <div
                className="nb-app-shell__profile-sidebar-title"
                data-nb-profile-sidebar-title
              >
                个人资料
              </div>
              <Button
            ...
            <div
              className="nb-app-shell__profile-sidebar-body"
              data-nb-profile-sidebar-body
            >
              <div
                className="nb-app-shell__profile-sidebar-avatar"
                data-nb-profile-sidebar-avatar
              >
                {shownAvatarUrl ? (
                  <div
                    style={{
              ...
              <div
                className="nb-app-shell__profile-sidebar-name"
                data-nb-profile-sidebar-name
              >
                {displayName || "未登录"}
              </div>
              {uid ? (
                <div
                  className="nb-app-shell__profile-sidebar-id"
                  data-nb-profile-sidebar-id
                >
                  {uid}
                </div>
              ) : null}
              ...
              {uid ? (
                <div className="nb-profile-editor" data-nb-profile-editor>
                  <div
                    className="nb-profile-editor__section"
                    data-nb-profile-editor-section
                  >
                    <div className="nb-profile-editor__label">头像</div>
                    <div
                      className="nb-profile-editor__row"
                      data-nb-profile-editor-row
                    >
                      <input
                      ...
                      <Button
                        ...
                        disabled={!avatarPreview || avatarSaving}
                      >
                        {avatarSaving ? "上传中…" : "上传"}
                      </Button>
                      {avatarPreview ? (
                      ...
                      <Button
                        onClick={clearAvatar}
                        disabled={!canClearAvatar || avatarSaving}
                      >
                        {avatarSaving ? "处理中…" : "清除"}
                      </Button>
                    </div>
                    {avatarError ? (
                      <div className="nb-profile-editor__error">
                        {avatarError}
                      </div>
                    ) : null}
                  </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <Button
                        ...
                        disabled={!canSaveDisplayName || savingProfile}
                      >
                  ...
                  >
                    ...
                    <div
                      className="nb-profile-editor__text"
                      data-nb-profile-editor-text
                    >
                      {friends.length
                        ? friends.join(", ")
                        : "暂无好友（可去“邀请好友”生成邀请码）"}
              ...
              <div
                className="nb-app-shell__profile-sidebar-actions"
                data-nb-profile-sidebar-actions
              >
                {!uid ? (
