...
export async function POST(req: Request) {
  ...
  if (visibility !== 'private' && visibility !== 'public') return NextResponse.json({ error: 'INVALID_VISIBILITY' }, { status: 400 });
  ...
  const result = await updateAppData((data) => {
    ...
    if (doc.kind !== 'template') return { ok: false as const, error: 'NOT_TEMPLATE' as const };
    ...
    return { ok: true as const, doc: { docId: doc.docId, kind: doc.kind, visibility: doc.visibility, updatedAtMs: doc.updatedAtMs } };
  });

  if (!result.ok) {
    const status = result.error === 'DOC_NOT_FOUND' ? 404 : result.error === 'FORBIDDEN' ? 403 : 400;
