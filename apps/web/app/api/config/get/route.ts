...
export async function GET(req: Request) {
  ...
  const visibility =
    doc.kind === "template" ? effectiveTemplateVisibility(doc) : undefined;
  const canEdit =
    doc.kind === "template" ? !!(uid && doc.ownerId === uid) : true;

  if (doc.kind === "template" && !canEdit) {
    if (visibility !== "public" || !doc.publishedVersionId)
      return NextResponse.json({ error: "DOC_NOT_FOUND" }, { status: 404 });
    const published = doc.publishedVersionId
      ? (doc.versions[doc.publishedVersionId] ?? null)
      : null;
