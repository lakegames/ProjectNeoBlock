...
export async function POST(req: Request) {
  ...
  const maxUses =
    typeof maxUsesRaw === "number" && Number.isFinite(maxUsesRaw)
      ? Math.trunc(maxUsesRaw)
      : 1;
  const expiresInSec =
    typeof expiresInSecRaw === "number" && Number.isFinite(expiresInSecRaw)
      ? Math.trunc(expiresInSecRaw)
      : 3600;
  ...
  const result = await updateAppData((data) => {
    ...
    return {
      code,
      link: origin ? `${origin}/invite/${code}` : `/invite/${code}`,
    };
  });
