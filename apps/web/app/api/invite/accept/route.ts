...
export async function POST(req: Request) {
  ...
  const result = await updateAppData((data) => {
    ...
    const expiresAtMs =
      invite.expiresAt && Number.isFinite(Date.parse(invite.expiresAt))
    ...
    const maxUses =
      typeof invite.maxUses === "number" && Number.isFinite(invite.maxUses)
        ? Math.trunc(invite.maxUses)
        : 1;
    const uses =
      typeof invite.uses === "number" && Number.isFinite(invite.uses)
        ? Math.trunc(invite.uses)
        : invite.usedBy
          ? 1
          : 0;
    if (uses >= maxUses)
    ...
    if (invite.inviterId === uid)
      return { ok: false as const, error: "CANNOT_ACCEPT_SELF" as const };
    ...
    data.friends[uid] = Array.from(
      new Set([...(data.friends[uid] ?? []), invite.inviterId]),
    );
