...
export async function POST(req: Request) {
  ...
  const result = await updateAppData((data) => {
    ...
    if (beforeCount === afterCount)
      return { ok: false as const, error: "NOT_IN_ROOM" as const };
    ...
    if (room.members.length === 0) {
      ...
      room.hostPlayerId =
        room.members.find((m) => !m.isSpectator)?.playerId ??
        room.members[0]?.playerId ??
        room.hostPlayerId;
    }
    ...
    appendAudit(data, {
      ...
      detail: {
        playerId,
        beforeHostPlayerId: beforeHost,
        afterHostPlayerId: deleted ? null : room.hostPlayerId,
        deleted,
      },
    });
    ...
    return {
      ok: true as const,
      deleted,
      hostPlayerId: deleted ? null : room.hostPlayerId,
    };
  });
