...
export async function GET(req: Request) {
  ...
  const users = ids
    .map((id) => {
      ...
      const roomCount = Object.values(data.rooms).filter((r) =>
        r.members.some((m) => m.userId === id),
      ).length;
      return {
        id,
        displayName: p?.displayName ?? id,
        ...(p
          ? avatarInfoFor(p as typeof p & { avatarKey?: string | null })
