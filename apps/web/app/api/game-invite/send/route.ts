...
export async function POST(req: Request) {
  ...
  const result = await updateAppData((data) => {
    ...
    if (!friends.includes(toUid))
      return { ok: false as const, error: "NOT_FRIEND" as const };
    ...
    if (data.gameInvites.length > 5000)
      data.gameInvites = data.gameInvites.slice(data.gameInvites.length - 5000);
