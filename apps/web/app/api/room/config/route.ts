...
export async function POST(req: Request) {
  ...
  const result = await updateAppData((data) => {
    ...
    if (!vr.ok) return { ok: false as const, error: 'INVALID_ROOM_CONFIG' as const, issues: vr.issues };

    const playerCount = room.members.filter((m) => !m.isSpectator).length;
    if (vr.value.maxPlayers < playerCount)
      return { ok: false as const, error: 'MAX_PLAYERS_TOO_SMALL' as const, playerCount };
    ...
    room.config = { ...vr.value, rulesetVersionId: rulesV.versionId, boardVersionId: boardV.versionId, cardsVersionId: cardsV.versionId };
    return { ok: true as const, room };
  });
