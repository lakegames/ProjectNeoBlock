...
export async function POST(req: Request) {
  ...
  if (!gameId) return NextResponse.json({ error: 'INVALID_GAME_ID' }, { status: 400 });
  ...
  const result = await updateAppData((data) => {
    ...
    if (!existing) {
      ...
      if (data.matchRecords.length > 5000) data.matchRecords = data.matchRecords.slice(data.matchRecords.length - 5000);
    }
  ...
  if (!result.ok) {
    const status = result.error === 'ROOM_NOT_FOUND' ? 404 : result.error === 'ROOM_CLOSED' ? 410 : result.error === 'NOT_HOST' ? 403 : 400;
