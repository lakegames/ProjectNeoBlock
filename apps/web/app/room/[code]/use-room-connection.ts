...
export function useRoomConnection(options: {
  ...
  const [lastCardDrawn, setLastCardDrawn] = React.useState<CardDrawn | null>(
    null,
  );
    ...
    (parsed: ServerMessage) => {
      ...
      if (parsed.kind === "events") {
        if (parsed.events.length > 0) {
          const maxSeq = parsed.events.reduce(
        ...
        if (parsed.events.length) {
          const next200 = [...recentEventsRef.current, ...parsed.events].slice(
            -200,
          );
        ...
        for (const e of parsed.events) {
          ...
          if (
            ce.toPlayerId &&
            selfId &&
            ce.fromPlayerId !== selfId &&
            ce.toPlayerId !== selfId
          )
            continue;
        ...
        if (pending) {
          const ok = parsed.events.some(
            (e) => e.causedBy?.commandId === pending.commandId,
          );
          if (ok) {
        ...
        const lastCard = [...parsed.events]
          ...
          .find(
            (e) =>
              e.type === "game/engine" &&
              (e as Extract<Event, { type: "game/engine" }>).name ===
                "card/drawn",
          );
        if (lastCard && lastCard.type === "game/engine") {
          const data = lastCard.data as {
            deck: "chance" | "communityChest";
            cardId: string;
            playerId: string;
          };
      ...
      if (parsed.kind === "error") {
        if (parsed.error.code === "OUT_OF_ORDER") {
          const expected = (
            parsed.error.details as { expectedClientSeq?: unknown } | undefined
          )?.expectedClientSeq;
          if (
            ...
            if (
              pending &&
              parsed.error.commandId === pending.commandId &&
              pending.retries < 1
            ) {
  ...
  const connect = React.useCallback(
    (a: Actor) => {
      ...
      ws.addEventListener("message", (ev) => {
        ...
        if (kind !== "snapshot" && kind !== "events" && kind !== "error")
          return;
