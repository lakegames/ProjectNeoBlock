...
export default function ConfigHomePage() {
  ...
  const [activeTab, setActiveTab] = useState<'template' | 'config'>(() => (searchParams.get('tab') === 'config' ? 'config' : 'template'));
  ...
  const [createTemplateRulesVersionId, setCreateTemplateRulesVersionId] = useState('');
  const [createTemplateBoardVersionId, setCreateTemplateBoardVersionId] = useState('');
  const [createTemplateCardsVersionId, setCreateTemplateCardsVersionId] = useState('');
  ...
  const grouped = useMemo(() => {
    const g: Record<'rules' | 'board' | 'cards' | 'template', DocListItem[]> = { rules: [], board: [], cards: [], template: [] };
  ...
  useEffect(() => {
    if (!createTemplateRulesVersionId) setCreateTemplateRulesVersionId(publishedRules[0]?.versionId ?? '');
  }, [createTemplateRulesVersionId, publishedRules]);
  useEffect(() => {
    if (createTemplateBoardVersionId) return;
    const full = publishedBoards.find((b) => b.docId === 'builtin:board-full')?.versionId ?? '';
    setCreateTemplateBoardVersionId(full || publishedBoards[0]?.versionId || '');
  }, [createTemplateBoardVersionId, publishedBoards]);
  useEffect(() => {
    if (!createTemplateCardsVersionId) setCreateTemplateCardsVersionId(publishedCards[0]?.versionId ?? '');
  ...
  async function createTemplate() {
    ...
    if (!createTemplateRulesVersionId || !createTemplateBoardVersionId || !createTemplateCardsVersionId) {
  ...
  return (
    <main style={{ padding: 24, maxWidth: 980 }}>
      ...
      <p style={{ marginTop: 8, color: 'rgba(0,0,0,0.65)' }}>创建模板（规则+棋盘+卡牌 的组合），以及管理规则 / 棋盘 / 卡牌 配置</p>

      <div style={{ marginTop: 16, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        <Button mode={activeTab === 'template' ? 'Primary' : 'Second'} onClick={() => setTab('template')}>
          模板
        </Button>
        <Button mode={activeTab === 'config' ? 'Primary' : 'Second'} onClick={() => setTab('config')}>
          配置
        </Button>
      ...
      {activeTab === 'template' ? (
        <>
          <section style={{ marginTop: 20, padding: 12, border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12 }}>
            ...
            <div style={{ marginTop: 12, display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
              <Input value={createTemplateName} onChange={(e) => setCreateTemplateName(e.target.value)} placeholder="模板名称，例如：标准玩法（快节奏）" style={{ minWidth: 280 }} />
              <select
                value={createTemplateRulesVersionId}
                onChange={(e) => setCreateTemplateRulesVersionId(e.target.value)}
                style={{ padding: '10px 12px', borderRadius: 12, border: '1px solid rgba(0,0,0,0.12)', background: '#fff', minWidth: 260 }}
              >
              ...
              <select
                ...
                onChange={(e) => setCreateTemplateBoardVersionId(e.target.value)}
                style={{ padding: '10px 12px', borderRadius: 12, border: '1px solid rgba(0,0,0,0.12)', background: '#fff', minWidth: 260 }}
              >
              ...
              <select
                ...
                onChange={(e) => setCreateTemplateCardsVersionId(e.target.value)}
                style={{ padding: '10px 12px', borderRadius: 12, border: '1px solid rgba(0,0,0,0.12)', background: '#fff', minWidth: 260 }}
              >
              ...
              <Button onClick={createTemplate} disabled={createTemplateLoading || !uid}>
                {createTemplateLoading ? '创建中…' : '创建'}
              </Button>
            </div>
            {!uid ? <div style={{ marginTop: 10, color: 'rgba(0,0,0,0.65)' }}>登录后才可创建“默认仅自己可见”的模板</div> : null}
            {createTemplateError ? <div style={{ marginTop: 10, color: '#b42318' }}>{createTemplateError}</div> : null}
          </section>

          <section style={{ marginTop: 20, padding: 12, border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12 }}>
            <div style={{ fontWeight: 700 }}>模板</div>
            <div style={{ marginTop: 10, color: 'rgba(0,0,0,0.65)' }}>模板是规则/棋盘/卡牌已发布版本的组合，可用于创建房间和房间参数</div>
                ...
                <ul style={{ margin: 0, paddingLeft: 18 }}>
                  {grouped.template.map((d) => (
                    <li key={d.docId} style={{ marginTop: 8 }}>
                      ...
                      {d.visibility ? (
                        <span style={{ marginLeft: 8, color: d.visibility === 'public' ? '#027a48' : 'rgba(0,0,0,0.65)', fontWeight: 700 }}>
                      ...
                      <Button style={{ marginLeft: 10 }} onClick={() => router.push(`/config/${encodeURIComponent(d.docId)}`)}>
                        进入
                      </Button>
                    </li>
                  ))}
        ...
        <>
          ...
          <section style={{ marginTop: 20, padding: 12, border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12 }}>
            ...
            <div style={{ marginTop: 10, color: 'rgba(0,0,0,0.65)' }}>点击进入后编辑草稿，发布后可用于模板与创建房间</div>
