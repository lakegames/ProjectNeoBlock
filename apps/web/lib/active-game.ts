...
export function readActiveGame(nowMs = Date.now()): ActiveGame | null {
  ...
  try {
    ...
    const updatedAtMs = typeof parsed.updatedAtMs === 'number' && Number.isFinite(parsed.updatedAtMs) ? parsed.updatedAtMs : 0;
...
export function writeActiveGame(roomCode: string, nowMs = Date.now()) {
  ...
  if (!code) return;
  window.localStorage.setItem(storageKey, JSON.stringify({ roomCode: code, updatedAtMs: nowMs } satisfies ActiveGame));
}
