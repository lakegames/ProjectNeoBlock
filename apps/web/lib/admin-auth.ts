...
type OrgCheckResult =
  ...
  | {
      ok: false;
      reason: "GITHUB_UNAUTHORIZED" | "GITHUB_FORBIDDEN" | "GITHUB_ERROR";
      status: number;
      message: string;
      oauthScopes: string | null;
    };
...
async function checkOrgMembership(
  ...
  if (r.status === 404)
    return { ok: false, reason: "NOT_ORG_MEMBER", status: 404, oauthScopes };
  if (r.status === 401) {
    return {
      ok: false,
  ...
  if (!accessToken) {
    return {
      ...
      res: NextResponse.json(
        { error: "FORBIDDEN", reason: "NO_GITHUB_ACCESS_TOKEN", org: adminOrg },
        { status: 403 },
      ),
    };
  }
  ...
  if (cached && now - cached.checkedAtMs < cacheTtlMs) {
    if (!cached.ok) {
      return {
        ok: false,
        res: NextResponse.json(
          { error: "FORBIDDEN", reason: "NOT_ORG_MEMBER", org: adminOrg },
          { status: 403 },
        ),
      };
    }
        ...
        res: NextResponse.json(
          {
            ...
            org: adminOrg,
            state: checked.state,
            oauthScopes: checked.oauthScopes,
          },
          { status: 403 },
        ),
      };
