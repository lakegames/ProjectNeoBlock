...
function isPublishedVersionId(
  data: AppData,
  kind: Exclude<ConfigKind, "template">,
  versionId: string,
) {
  return Object.values(data.configDocs).some(
    (d) => d.kind === kind && d.publishedVersionId === versionId,
  );
}

export function resolveTemplateByPublishedVersionId(
  data: AppData,
  versionId: string,
): TemplateConfig | null {
  const doc =
    Object.values(data.configDocs).find(
      (d) => d.kind === "template" && d.publishedVersionId === versionId,
    ) ?? null;
  if (!doc?.publishedVersionId) return null;
  const payload = doc.versions[doc.publishedVersionId]?.data ?? null;
  const vr = validateTemplateConfig(payload);
  if (!vr.ok) return null;
  if (!isPublishedVersionId(data, "rules", vr.value.rulesVersionId))
    return null;
  if (!isPublishedVersionId(data, "board", vr.value.boardVersionId))
    return null;
  if (!isPublishedVersionId(data, "cards", vr.value.cardsVersionId))
    return null;
  return vr.value;
}

export function resolveTemplateDocByPublishedVersionId(
  data: AppData,
  versionId: string,
): ConfigDoc | null {
  return (
...
function validateTemplateRefs(
  ...
  if (!isPublishedVersionId(data, "rules", input.rulesVersionId))
    issues.push({
      path: "rulesVersionId",
      message: "rulesVersionId 必须为已发布版本",
    });
  if (!isPublishedVersionId(data, "board", input.boardVersionId))
    issues.push({
      path: "boardVersionId",
      message: "boardVersionId 必须为已发布版本",
    });
  if (!isPublishedVersionId(data, "cards", input.cardsVersionId))
    issues.push({
      path: "cardsVersionId",
      message: "cardsVersionId 必须为已发布版本",
    });
  return issues;
}
...
export function createNewDoc(
  ...
  const doc = createDoc({
    ...
    name: input.name,
    ...(input.kind === "template"
      ? { ownerId: input.ownerId ?? null, visibility: "private" as const }
      : {}),
    nowMs: input.nowMs,
    draftData,
  });
...
export function updateDraft(
  data: AppData,
  input: { docId: string; nowMs: number; draftData: unknown },
): UpdateDraftResult {
  ...
  if (!vr.ok)
    return { ok: false, error: "VALIDATION_FAILED", issues: vr.issues };

  if (doc.kind === "template") {
    const refIssues = validateTemplateRefs(data, vr.value as TemplateConfig);
    if (refIssues.length)
      return { ok: false, error: "VALIDATION_FAILED", issues: refIssues };
  }
...
export function publishByDocId(
  data: AppData,
  input: { docId: string; nowMs: number; note?: string },
): PublishResult {
  const doc = data.configDocs[input.docId];
  if (!doc) return { ok: false, error: "DOC_NOT_FOUND" };
...
export function rollbackByDocId(
  data: AppData,
  input: {
    docId: string;
...
export function resolvePublishedPayload(
  data: AppData,
  kind: "rules",
): RulesConfig | null;
export function resolvePublishedPayload(
