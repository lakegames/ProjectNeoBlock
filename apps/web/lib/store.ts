...
function coerceRoomConfig(input: unknown): RoomConfig {
  ...
  return {
    ...
    ...(typeof v.templateVersionId === "string" && v.templateVersionId
      ? { templateVersionId: v.templateVersionId }
      : {}),
    ...(typeof v.rulesetVersionId === "string" && v.rulesetVersionId
      ? { rulesetVersionId: v.rulesetVersionId }
      : {}),
    ...(typeof v.boardVersionId === "string" && v.boardVersionId
      ? { boardVersionId: v.boardVersionId }
      : {}),
    ...(typeof v.cardsVersionId === "string" && v.cardsVersionId
      ? { cardsVersionId: v.cardsVersionId }
      : {}),
  };
}
...
function coerceConfigVersion(input: unknown) {
  ...
  const versionId =
    typeof r.versionId === "string" && r.versionId ? r.versionId : null;
  const status =
    r.status === "draft" || r.status === "published" || r.status === "archived"
      ? r.status
      : null;
  ...
  const baseVersionId =
    typeof r.baseVersionId === "string" && r.baseVersionId
      ? r.baseVersionId
      : undefined;
...
function coerceConfigDoc(input: unknown, docIdKey: string): ConfigDoc | null {
  ...
  const versionsRaw =
    typeof (r as { versions?: unknown }).versions === "object" &&
    (r as { versions?: unknown }).versions !== null
      ? ((r as { versions: Record<string, unknown> }).versions as Record<
          string,
          unknown
        >)
      : {};
  ...
  const versionIds =
    ...
    r.versionIds.every((x) => typeof x === "string")
      ? (r.versionIds as string[])
      : Object.keys(versions);

  const draftVersionId =
    typeof r.draftVersionId === "string"
      ? r.draftVersionId
      : (versionIds[0] ?? "");
...
function coerceRoom(input: unknown, roomCode: string): Room | null {
  ...
  const roomId =
    typeof r.roomId === "string" && r.roomId ? r.roomId : `room:${code}`;
  const status =
    r.status === "lobby" || r.status === "playing" || r.status === "ended"
      ? r.status
      : ("lobby" as const);
  const createdAtMs = coerceNumber(r.createdAtMs, Date.now());
  const startedAtMs =
    typeof r.startedAtMs === "number" && Number.isFinite(r.startedAtMs)
      ? r.startedAtMs
      : undefined;
  const endedAtMs =
    typeof r.endedAtMs === "number" && Number.isFinite(r.endedAtMs)
      ? r.endedAtMs
      : undefined;
  const emptySinceMs =
    typeof r.emptySinceMs === "number" && Number.isFinite(r.emptySinceMs)
      ? r.emptySinceMs
      : undefined;
  const closedAtMs =
    typeof r.closedAtMs === "number" && Number.isFinite(r.closedAtMs)
      ? r.closedAtMs
      : undefined;
  ...
  if (Array.isArray(r.members)) {
    const members: RoomMember[] = r.members
      .map((m) => {
        ...
        const displayName =
          typeof mm.displayName === "string" ? mm.displayName : playerId;
    ...
    const hostPlayerId =
      typeof r.hostPlayerId === "string" && r.hostPlayerId
        ? r.hostPlayerId
        : members[0]?.playerId || "unknown";
    return {
      code,
      roomId,
      status,
  ...
  if (Array.isArray(r.participants)) {
    ...
    const members: RoomMember[] = participants.map((p) => ({
      ...
      isSpectator: false,
      ready: false,
      joinedAtMs: Number.isFinite(Date.parse(p.joinedAt))
...
function coerceMatchRecord(input: unknown): MatchRecord | null {
  ...
  const roomCode =
    typeof r.roomCode === "string" && r.roomCode
  ...
  if (!id || !roomCode || !gameId || !roomId || !hostPlayerId || !endedAtMs)
    return null;

  const configSnapshot = r.configSnapshot
    ? coerceRoomConfig(r.configSnapshot)
    : defaultRoomConfig();

  const participantsRaw = Array.isArray(r.participants)
  ...
  const hiddenByUids =
    ...
    (r as { hiddenByUids: unknown[] }).hiddenByUids.every(
      (x) => typeof x === "string",
    )
  ...
  try {
    ...
    const roomsRaw =
      ...
      (parsed as unknown as { rooms?: unknown }).rooms !== null
        ? ((parsed as unknown as { rooms: Record<string, unknown> })
    ...
    const configDocsRaw =
      typeof (parsed as unknown as { configDocs?: unknown }).configDocs ===
        "object" &&
      (parsed as unknown as { configDocs?: unknown }).configDocs !== null
        ? ((parsed as unknown as { configDocs: Record<string, unknown> })
            .configDocs as Record<string, unknown>)
        : {};
    ...
    const auditRaw = Array.isArray(
      (parsed as unknown as { audit?: unknown }).audit,
    )
    ...
    const audit: AuditEntry[] = auditRaw
      .map((x) => {
        ...
        const actorUid =
          typeof r.actorUid === "string" && r.actorUid ? r.actorUid : null;
        const action =
          typeof r.action === "string" && r.action ? r.action : null;
        if (!id || !actorUid || !action || !atMs) return null;
        const targetType =
          typeof r.targetType === "string" && r.targetType
            ? r.targetType
            : undefined;
        const targetId =
          typeof r.targetId === "string" && r.targetId ? r.targetId : undefined;
    ...
    const gameInvitesRaw = Array.isArray(
      (parsed as unknown as { gameInvites?: unknown }).gameInvites,
    )
      ? ((parsed as unknown as { gameInvites: unknown[] })
          .gameInvites as unknown[])
      : [];
    const gameInvites: GameInviteMessage[] = gameInvitesRaw
      .map((x) => {
        ...
        const fromUid =
          typeof r.fromUid === "string" && r.fromUid ? r.fromUid : null;
        const roomCode =
          typeof r.roomCode === "string" && r.roomCode ? r.roomCode : null;
        const createdAtMs = coerceNumber(r.createdAtMs, 0);
        if (!id || !toUid || !fromUid || !roomCode || !createdAtMs) return null;
        const readAtMs =
          typeof r.readAtMs === "number" && Number.isFinite(r.readAtMs)
            ? r.readAtMs
            : undefined;
        const dismissedAtMs =
          typeof r.dismissedAtMs === "number" &&
          Number.isFinite(r.dismissedAtMs)
            ? r.dismissedAtMs
            : undefined;
    ...
    const matchRecordsRaw = Array.isArray(
      (parsed as unknown as { matchRecords?: unknown }).matchRecords,
    )
      ? ((parsed as unknown as { matchRecords: unknown[] })
          .matchRecords as unknown[])
      : [];
...
export async function updateAppData<T>(
