...
export function validateTemplateConfig(
  input: unknown,
): ConfigValidationResult<TemplateConfig> {
  if (!isRecord(input))
    return { ok: false, issues: [issue("", "模板配置必须为对象")] };
  const raw = input as Record<string, unknown>;
  const issues: ConfigIssue[] = [];
  const requireString = (key: keyof TemplateConfig) => {
    const v = raw[key];
    if (typeof v !== "string" || !v.trim())
      issues.push(issue(String(key), `${String(key)} 必须为非空字符串`));
  ...
  return {
    ok: true,
    value: { rulesVersionId, boardVersionId, cardsVersionId },
  };
}

export function validateRulesConfig(
  input: unknown,
): ConfigValidationResult<RulesConfig> {
  if (!isRecord(input))
    return { ok: false, issues: [issue("", "规则配置必须为对象")] };
  const issues: ConfigIssue[] = [];
  const raw = input as Record<string, unknown>;

  const initialCash = clampInt(Number(raw.initialCash), 100, 50_000);
  if (
    !Number.isFinite(Number(raw.initialCash)) ||
    initialCash !== Math.trunc(Number(raw.initialCash))
  )
    issues.push(issue("initialCash", "initialCash 必须为整数"));

  const startSalary = clampInt(Number(raw.startSalary), 0, 50_000);
  if (
    !Number.isFinite(Number(raw.startSalary)) ||
    startSalary !== Math.trunc(Number(raw.startSalary))
  )
    issues.push(issue("startSalary", "startSalary 必须为整数"));

  const jailFine = clampInt(Number(raw.jailFine), 0, 50_000);
  if (
    !Number.isFinite(Number(raw.jailFine)) ||
    jailFine !== Math.trunc(Number(raw.jailFine))
  )
    issues.push(issue("jailFine", "jailFine 必须为整数"));

  const mortgageInterestRate = clampNumber(
    Number(raw.mortgageInterestRate),
    0,
    1,
  );
  if (!Number.isFinite(Number(raw.mortgageInterestRate)))
    issues.push(
      issue("mortgageInterestRate", "mortgageInterestRate 必须为数字"),
    );
  if (mortgageInterestRate < 0 || mortgageInterestRate > 1)
    issues.push(
      issue("mortgageInterestRate", "mortgageInterestRate 范围为 0-1"),
    );

  const bankHouses = clampInt(Number(raw.bankHouses), 0, 200);
  if (
    !Number.isFinite(Number(raw.bankHouses)) ||
    bankHouses !== Math.trunc(Number(raw.bankHouses))
  )
    issues.push(issue("bankHouses", "bankHouses 必须为整数"));

  const bankHotels = clampInt(Number(raw.bankHotels), 0, 200);
  if (
    !Number.isFinite(Number(raw.bankHotels)) ||
    bankHotels !== Math.trunc(Number(raw.bankHotels))
  )
...
function validateTile(
  input: unknown,
  index: number,
): ConfigValidationResult<BoardTileTemplate> {
  if (!isRecord(input))
    return { ok: false, issues: [issue(`tiles.${index}`, "tile 必须为对象")] };
  const raw = input as Record<string, unknown>;
  const kind = raw.kind;
  if (!isString(kind))
    return {
      ok: false,
      issues: [issue(`tiles.${index}.kind`, "kind 必须为字符串")],
    };
  ...
  const requireString = (key: string) => {
    ...
    if (!isString(v) || v.trim().length === 0)
      issues.push(issue(`${base}.${key}`, `${key} 必须为非空字符串`));
    return String(v ?? "");
  };
  ...
  const requireInt = (key: string, min: number, max: number) => {
    ...
    if (!Number.isFinite(v) || Math.trunc(v) !== v)
      issues.push(issue(`${base}.${key}`, `${key} 必须为整数`));
    const c = clampInt(v, min, max);
    if (c < min || c > max)
      issues.push(issue(`${base}.${key}`, `${key} 范围为 ${min}-${max}`));
    return c;
  };
  ...
  if (kind === "communityChest")
    return { ok: true, value: { kind: "communityChest" } };
  ...
  if (kind === "property") {
    ...
    if (nameRaw !== undefined) {
      if (!isString(nameRaw) || nameRaw.trim().length === 0)
        issues.push(issue(`${base}.name`, "name 必须为非空字符串"));
      else if (nameRaw.trim().length > 32)
        issues.push(issue(`${base}.name`, "name 最大长度为 32"));
    ...
    if (groupNameRaw !== undefined) {
      if (!isString(groupNameRaw) || groupNameRaw.trim().length === 0)
        issues.push(issue(`${base}.groupName`, "groupName 必须为非空字符串"));
      else if (groupNameRaw.trim().length > 32)
        issues.push(issue(`${base}.groupName`, "groupName 最大长度为 32"));
    ...
    const rentsParsed = (Array.isArray(rentsRaw) ? rentsRaw : []).map(
      (x, i) => {
        const n = Number(x);
        if (!Number.isFinite(n) || Math.trunc(n) !== n)
          issues.push(issue(`${rentsBase}.${i}`, "租金必须为整数"));
        if (n < 0 || n > 50_000)
          issues.push(issue(`${rentsBase}.${i}`, "租金范围为 0-50000"));
        return clampInt(n, 0, 50_000);
      },
    );

    if (issues.length) return { ok: false, issues };
    const rents = rentsParsed.slice(0, 6) as [
      number,
      number,
      number,
      number,
...
export function validateBoardConfig(
  input: unknown,
): ConfigValidationResult<BoardConfigTemplate> {
  if (!isRecord(input))
    return { ok: false, issues: [issue("", "棋盘配置必须为对象")] };
  const raw = input as Record<string, unknown>;
  const issues: ConfigIssue[] = [];

  const tilesRaw = raw.tiles;
  if (!Array.isArray(tilesRaw) || tilesRaw.length === 0)
    issues.push(issue("tiles", "tiles 必须为非空数组"));
  ...
  const jailIndex = clampInt(
    Number(raw.jailIndex),
    0,
    Math.max(0, tilesParsed.length - 1),
  );
  if (
    !Number.isFinite(Number(raw.jailIndex)) ||
    jailIndex !== Math.trunc(Number(raw.jailIndex))
  )
    issues.push(issue("jailIndex", "jailIndex 必须为整数"));
  if (
    tilesParsed.length > 0 &&
    (jailIndex < 0 || jailIndex >= tilesParsed.length)
  )
  ...
  tilesParsed.forEach((t, idx) => {
    ...
    if (propertyIds.has(pid))
      issues.push(issue(`tiles.${idx}.propertyId`, "propertyId 重复"));
    propertyIds.add(pid);
  });
...
function validateCardEffect(
  ...
  if (kind === "money") {
    ...
    if (!Number.isFinite(delta) || Math.trunc(delta) !== delta)
      issues.push(issue(`${base}.delta`, "delta 必须为整数"));
  ...
  if (kind === "moneyFromEachPlayer") {
    ...
    if (!Number.isFinite(amount) || Math.trunc(amount) !== amount)
      issues.push(issue(`${base}.amount`, "amount 必须为整数"));
    return { kind: "moneyFromEachPlayer", amount: clampInt(amount, 0, 50_000) };
  }

  if (kind === "moveTo") {
    const index = Number(effect.index);
    if (!Number.isFinite(index) || Math.trunc(index) !== index)
      issues.push(issue(`${base}.index`, "index 必须为整数"));
    const passStart = effect.passStart;
    if (!isBoolean(passStart))
      issues.push(issue(`${base}.passStart`, "passStart 必须为 boolean"));
    return {
      kind: "moveTo",
      index: clampInt(index, 0, 10_000),
      passStart: isBoolean(passStart) ? passStart : false,
    };
  }
...
export function validateCardsConfig(
  input: unknown,
): ConfigValidationResult<CardsConfig> {
  if (!isRecord(input))
    return { ok: false, issues: [issue("", "卡牌配置必须为对象")] };
  const raw = input as Record<string, unknown>;
  const issues: ConfigIssue[] = [];
  ...
  if (Array.isArray(cardsRaw)) {
    for (let i = 0; i < cardsRaw.length; i += 1) {
      ...
      if (!isString(cardId) || cardId.trim().length === 0)
        issues.push(issue(`${base}.cardId`, "cardId 必须为非空字符串"));
      const deck = c.deck;
      if (deck !== "chance" && deck !== "communityChest")
        issues.push(
          issue(`${base}.deck`, "deck 必须为 chance 或 communityChest"),
        );
      const text = c.text;
      if (!isString(text) || text.trim().length === 0)
        issues.push(issue(`${base}.text`, "text 必须为非空字符串"));
      const eff = validateCardEffect(c.effect, `${base}.effect`, issues);
      if (!eff) continue;
      if (
        isString(cardId) &&
        (deck === "chance" || deck === "communityChest") &&
        isString(text)
      ) {
  ...
  parsed.forEach((c, idx) => {
    if (ids.has(c.cardId))
      issues.push(issue(`cards.${idx}.cardId`, "cardId 重复"));
    ids.add(c.cardId);
  });
