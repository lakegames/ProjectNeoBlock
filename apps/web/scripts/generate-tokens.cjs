...
function resolveTokenValue(token) {
  ...
  if (ext && typeof ext.targetVariableName === 'string' && ext.targetVariableName.length > 0) {
...
function main() {
  ...
  const lightFiles = [path.join(tokenDir, 'Light.tokens.json'), path.join(tokenDir, 'control', 'light.tokens.json')];
  const darkFiles = [path.join(tokenDir, 'Dark.tokens.json'), path.join(tokenDir, 'control', 'deep.tokens.json')];
  ...
  const neoblockThemeVars = [
    { name: 'nb-color-bg', value: 'var(--control-normal-lightBackground-white)' },
    { name: 'nb-color-surface', value: 'var(--control-normal-lightBackground-whiteOnly)' },
    ...
    { name: 'nb-color-muted', value: 'var(--control-normal-lightBackground-white)' },
    ...
    { name: 'nb-color-primary-soft', value: 'var(--control-theme-lightBackground)' },
    ...
    { name: 'nb-color-special-soft', value: 'var(--box-infomation-lightBackground)' },
    { name: 'nb-color-special-soft-hover', value: 'var(--info-2)' },
    { name: 'nb-color-special-soft-active', value: 'var(--info-3)' },
    { name: 'nb-color-special-ink', value: 'var(--text-infomation-primary-black)' },
    { name: 'nb-color-input-bg', value: 'var(--control-normal-lightBackground-white)' },
    { name: 'nb-color-ring', value: 'var(--control-theme-lightBackground)' },
  ];

  const commonWithNeoblockTheme = [...common, ...neoblockThemeVars].sort((a, b) => a.name.localeCompare(b.name));
